...
workflow HaplotypeCallerGvcf_GATK4 {
  ...
  call GenotypeGVCFs_single {
    input: 
      input_bam = if (reference == "mm10") then MergeGVCFs.output_vcf else HaplotypeCaller_other.output_vcf,
      input_bam_index = if (reference == "mm10") then MergeGVCFs.output_vcf_index else HaplotypeCaller_other.output_vcf_index,
